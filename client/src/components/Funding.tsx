...
const content = {
  zh: {
    badge: '07 / 融资计划',
    title: '融资计划：$6M 种子轮，聚焦生态构建',
    subtitle: 'Strategic capital allocation for platform growth',
    desc: '种子轮融资 $6M，重点投入产品开发、创作者激励与品牌客户获取，建立平台的初始网络效应。',
    fundingData: [
  ...
  en: {
    badge: '07 / Funding Plan',
    title: 'Funding Plan: $6M Seed Round, Focused on Ecosystem Building',
    subtitle: 'Strategic capital allocation for platform growth',
    desc: '$6M seed round focused on product development, creator incentives, and brand customer acquisition to establish the platform\'s initial network effects.',
    fundingData: [
      { label: 'Product & Technology', value: 35, color: 'from-blue-400 to-blue-600' },
      { label: 'Marketing & Operations', value: 25, color: 'from-cyan-400 to-cyan-600' },
      { label: 'Creator Incentives', value: 18, color: 'from-purple-400 to-purple-600' },
      { label: 'Brand Customer Acquisition', value: 12, color: 'from-green-400 to-green-600' },
      { label: 'International Expansion', value: 6, color: 'from-orange-400 to-orange-600' },
      { label: 'Operating Reserve', value: 4, color: 'from-pink-400 to-pink-600' },
    ],
    metrics: [
      { label: 'Target within 18 months', value: 'DAU 100K+', desc: 'Daily Active Users' },
      { label: 'Creator Network Scale', value: '10K+', desc: 'Professional & Part-time Creators' },
      { label: 'Brand Clients', value: '100+', desc: 'Annual Contract Clients' },
    ],
    budgetOf: '% of budget',
  ...
  return (
    <section id="funding" className="relative py-20 lg:py-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid lg:grid-cols-[0.88fr_1.12fr] gap-8 lg:gap-10 items-start">
            ...
            <div className="relative w-56 h-56 sm:w-64 sm:h-64 lg:w-72 lg:h-72">
              <svg viewBox="0 0 200 200" className="w-full h-full">
                ...
                <circle cx="100" cy="100" r="80" fill="none" stroke="url(#gradient2)" strokeWidth="30"
                  ...
                  strokeDashoffset={`${(-35 * 2 * Math.PI * 80) / 100}`} transform="rotate(-90 100 100)" />
                <circle cx="100" cy="100" r="80" fill="none" stroke="url(#gradient3)" strokeWidth="30"
                  strokeDasharray={`${(18 * 2 * Math.PI * 80) / 100} ${2 * Math.PI * 80}`}
                  strokeDashoffset={`${(-(35 + 25) * 2 * Math.PI * 80) / 100}`} transform="rotate(-90 100 100)" />
                <circle cx="100" cy="100" r="80" fill="none" stroke="url(#gradient4)" strokeWidth="30"
                  strokeDasharray={`${(12 * 2 * Math.PI * 80) / 100} ${2 * Math.PI * 80}`}
                  strokeDashoffset={`${(-(35 + 25 + 18) * 2 * Math.PI * 80) / 100}`} transform="rotate(-90 100 100)" />
                <circle cx="100" cy="100" r="80" fill="none" stroke="url(#gradient5)" strokeWidth="30"
                  strokeDasharray={`${(6 * 2 * Math.PI * 80) / 100} ${2 * Math.PI * 80}`}
                  strokeDashoffset={`${(-(35 + 25 + 18 + 12) * 2 * Math.PI * 80) / 100}`} transform="rotate(-90 100 100)" />
                <circle cx="100" cy="100" r="80" fill="none" stroke="url(#gradient6)" strokeWidth="30"
                  strokeDasharray={`${(4 * 2 * Math.PI * 80) / 100} ${2 * Math.PI * 80}`}
                  strokeDashoffset={`${(-(35 + 25 + 18 + 12 + 6) * 2 * Math.PI * 80) / 100}`} transform="rotate(-90 100 100)" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <div className="text-xs text-slate-400 mt-1">{T.seedRound}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {fundingData.map((item, i) => (
              <div key={i} className="bg-gradient-to-br from-white/9 to-white/5 border border-white/14 backdrop-blur-xl rounded-lg p-3 sm:p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full bg-gradient-to-br ${item.color} flex-shrink-0`} />
                  <div>
                    <div className="font-bold text-white text-sm">{item.label}</div>
                    <div className="text-xs text-slate-400">{item.value}{T.budgetOf}</div>
                  </div>
                </div>
                <div className="text-base sm:text-lg font-black text-white">${(item.value * 0.06).toFixed(1)}M</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Key Metrics */}
        <div className="mt-12 lg:mt-16 grid sm:grid-cols-3 gap-4 sm:gap-6 lg:gap-7">
          {T.metrics.map((metric, i) => (
            <div key={i} className="bg-gradient-to-br from-white/9 to-white/5 border border-white/14 backdrop-blur-xl rounded-2xl p-5 sm:p-6 lg:p-8 text-center">
